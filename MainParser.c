...
/* External objects */
extern jupiter_intg syntaxErrorNumber /* number of syntax errors reported by the parser */;
extern jupiter_intg line; /* source code line number - defined in scanner.c */
...
extern jupiter_void startParser(jupiter_void);
extern jupiter_intg startScanner(ReaderPointer sc_buf);

static jupiter_void printParserError(jupiter_char* fmt, ...);
static jupiter_void displayParser(ReaderPointer ptrBuffer);
static jupiter_long getParserFileSize(jupiter_char* fname);
static jupiter_void callGarbageCollector(jupiter_void);
...
jupiter_void printParserError(jupiter_char* fmt, ...) {

	va_list ap;
	va_start(ap, fmt);

	(jupiter_void)vfprintf(stderr, fmt, ap);
	va_end(ap);
...
jupiter_long getParserFileSize(jupiter_char* fname) {
	FILE* input;
	jupiter_long flength;
	input = fopen(fname, "r");
	if (input == NULL) {
...
jupiter_void displayParser(ReaderPointer ptrBuffer) {
...
jupiter_void callGarbageCollector(jupiter_void) {
