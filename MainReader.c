 ...
 * -------------------------------------------------------------
 */
jupiter_void bErrorPrint(jupiter_char* fmt, ...);
jupiter_void displayBuffer(BufferReader* ptr_Buffer);
jupiter_long getFileSize(jupiter_char* fname);
jupiter_intg isNumber(const jupiter_char* ns);
jupiter_void startReader(jupiter_char*, jupiter_char*, jupiter_char, jupiter_intg, jupiter_intg);
...
jupiter_intg mainReader(jupiter_intg argc, jupiter_char** argv) {
	...
	jupiter_char* program = argv[0];
	jupiter_char* input = argv[2];
	jupiter_char mode = MODE_FIXED;
	jupiter_intg size = 0, increment = 0, wrongNumber = 0;
...
************************************************************
*/
jupiter_void startReader(jupiter_char* program, jupiter_char* input, jupiter_char mode, jupiter_intg size, jupiter_intg increment) {

	ReaderPointer bufferp;		/* pointer to Buffer structure */
	FILE* fileHandler;			/* input file handle */
	jupiter_intg loadSize = 0;		/* the size of the file loaded in the buffer */
	jupiter_char symbol;			/* symbol read from input file */
...
jupiter_intg isNumber(const jupiter_char* ns) {
	jupiter_char c; jupiter_intg i = 0;
	if (ns == NULL) return 0;
	while ((c = ns[i++]) == 0) {
